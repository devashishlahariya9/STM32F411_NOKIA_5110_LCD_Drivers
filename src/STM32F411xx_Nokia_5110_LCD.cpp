/*
 * STM32F411xx_Nokia_5110_LCD.cpp
 *
...
void Nokia_LCD :: delay_ms(int del)
{
	for(int i=0; i<(del*1600); i++);
}

void Nokia_LCD :: reset_lcd()
{
	GPIOB -> ODR &= ~(1 << reset);
	delay_ms(10);
	GPIOB -> ODR |= (1 << reset);
...
void Nokia_LCD :: datawrt(unsigned char data)
{
	while(!(SPI1 -> SR & (1 << 1)));
...
void Nokia_LCD :: command(unsigned char data)
{
...
void Nokia_LCD :: lcd_dataWrite(unsigned char data)
{
...
void Nokia_LCD :: setCursor(short int x, short int y)
{
	command(0x40 | y);
	command(0x80 | x);
}

void Nokia_LCD :: print(char *ptr)
{
	for(; *ptr != '\0'; ptr++)
	{
...
void Nokia_LCD :: print(double num)
{
	char arr[20];
	sprintf(arr, "%.2f", num);
	print(arr);
...
void Nokia_LCD :: display()
{
	spi1.endTransmission();
}

void Nokia_LCD :: print_bitmap(char b0, char b1, char b2, char b3, char b4, char b5, char b6, char b7)
{
